<p align="center">
  ...
  <img src="images/plugin-logo.png" alt="Advanced Achievements" width="180" />
</p>
...
- **12 Task Types:** Tracks block breaking, item pickup, mob kills, crafting, fishing, eating, enchanting, trading, mining, breeding, taming, and player death â€” each with support for `ANY` targets or specific material/entity names
- **Reward System:** Five reward types supported per achievement â€” items, XP, economy money (via Vault), server commands with `{player}` and `{uuid}` placeholders, and title messages â€” all defined directly in `achievements.yml`
...
5. Start the server â€” the plugin generates `plugins/AdvancedAchievements/` containing `config.yml`, `achievements.yml`, and `messages.yml`
...
**`config.yml`** â€” Global plugin settings
...
| `database.mysql.*` | â€” | MySQL host, port, database, username, password |
...
**`achievements.yml`** â€” All achievement definitions (see Achievement Definition Format below)

**`messages.yml`** â€” All plugin messages, prefixes, and GUI item names (fully supports `&` colour codes)
...
- `{player}` â€” replaced with the player's name
- `{uuid}` â€” replaced with the player's UUID
...
| `amount` | Integer â‰¥ 1 | `/ach edit first_steps amount 25` |
...
- **Economy (optional):** [Vault](https://www.spigotmc.org/resources/vault.34315/) â€” required only for `MONEY:` reward type; all other reward types work without Vault
...
// Get progress percentage (0.0 â€“ 1.0)
...
**Custom Events:**
- `AchievementUnlockEvent` â€” fired when a player unlocks an achievement (cancellable)
- `AchievementProgressEvent` â€” fired whenever a player's progress increases
